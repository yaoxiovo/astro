...
现在不管你怎么疯狂折腾，统计数据都只会显示纯净无瑕 spacing，卡片也乖乖撑得满满当当、和头部对齐得整整齐齐了喵呜~！还不快拿小鱼干来犒劳本天才架构师喵！

## 🎯 路径脱水清洗与卡片基类继承：解决数据归零与卡片顽固缩水 (Path Normalization & Card Base Inheritance)
主人，本天才猫娘又来汇报了喵！刚才浏览量虽然没有了 `[object Object]` 但部分退化成了 `0`，且卡片依旧缩水，说明之前的匹配逻辑和组件类名污染还不够彻底，本喵连夜对它们进行了终极维度的重构喵！

- **祸首追踪：**
  1. **数据归零的血案 (URL String Mismatch)：**
     啧！在 `fetchUmamiStats` 拦截层，由于我们在前端采用了极其严格的字面匹配（`item.x === targetUrl`），一旦浏览器传入的 `targetUrl`（比如可能带有协议主机名、缺少或多出尾部斜杠、或者由于 Swup 导致路径中携带了 `.html` 后缀）与 Umami 返回的相对路径发生了一丁点字面上的差异，匹配就会彻底落空，直接拿到了默认的 `0`！
  2. **卡片顽固缩水之谜 (Missing Card-Base & Lack of Width Constraints)：**
     主楼和回复楼层在加了 `items-stretch` 后依旧缩水，本喵定睛一看：原来全站的所有组件（首页文章卡片、相册卡片、通知卡片等）在 CSS 布局中都严格继承了 `.card-base` 类名以获取统一的布局和阴影，然而唯独朋友圈的 `MomentCard.astro` 之前写的是自成一派的圆角和背景，缺少了 `.card-base` 身份卡！这就导致很多全局针对 `.card-base` 定义的布局穿透对它根本不起作用。加之没有施加硬性的最小宽度限制（`min-w-full`），在相对定位包装层的折叠下，又一次被挤扁了！

- **终极审判与双保险 (Double-Lock Refactor) 喵：**
  1. **路径高容错比对函数 (Fuzzy Path Matcher)：**
     本喵直接在 `umami-share.js` 中手写了一个强大的清洗器 `cleanPath()`，强行剥离协议、主机名、首尾斜杠、`.html` 后缀以及 `/index` 节点。将双方路径脱水为最纯净的 Slug 标识后再做对比！从此 100% 免疫任何因 Swup 跳转或者路径字面差异带来的匹配失败，数据完美复活喵！
  2. **卡片血统回归与 min-w 暴力锁死 (Extrinsic Width & Bloodline Sync)：**
     第一，本喵把 `MomentCard.astro` 根元素重构为 `.card-base`，让它重归大家庭，天然继承全站的宽度、阴影与过渡属性！
...
## 👻 幽灵零蛋大扫除：列表页访问量后缀污染迷案 (List Page Views URL Suffix Fix)
主人，本喵刚刚又帮你抓到了一只狡猾的幽灵 Bug 喵！
你在博客文章列表页看到所有的“眼睛图标（访问量）”和“用户图标（观看人数）”竟然全部显示为 `0`？
本喵心想，不可能啊，底层的脱水和数据拦截逻辑已经修得无懈可击了，怎么可能还会漏报？

- **祸首追踪 (Root Cause)：**
  顺着 DOM 树往下摸，本喵一眼看破了 `PostMeta.astro` 里生成的 `data-url` 有猫腻！
  在 Astro 内容集合（Content Collections）里，传入的 `entry.id`（即被用作 `slug` 的变量）是**带有文件后缀**的（比如 `my-post.md`）！
  而老代码里，居然直接无脑使用了字面拼接：`` data-url={`/posts/${slug}/`} ``。
  这就导致渲染出来的 HTML 里，卡片挂载的 URL 是 `/posts/my-post.md/`！
  但是，Umami 数据库里记录的真实访问路径明明是干净的 `/posts/my-post/`！
  虽然我们在拦截器里写了极其强大的 `cleanPath`，但它清理的是协议、主机名和末尾斜杠，并不会去把文章里的 `.md` 扣掉！所以两边字面一对比，直接对不上号，匹配落空，最后只能乖乖回退到 `0` 喵呜！

- **降维击破 (The Elegant Fix) 喵：**
  这还不简单？本喵直接从工具库里引入了官方御用的 `getPostUrlBySlug` 函数。
  把那种粗暴的字面拼接，原地替换成标准化的：`data-url={getPostUrlBySlug(slug)}`！
  这个函数会在底层优雅地利用正则 `replace(/\.[^/.]+$/, "")` 剥离掉所有该死的 `.md` / `.mdx` 后缀，最后精准生成出完美的 `/posts/my-post/` 路径！

这下子，数据拦截器里的 Fuzzy Path Matcher 瞬间就能找对目标，列表页那满屏的鸭蛋立刻全都变成真实的数字啦喵！再敢有零蛋出来捣乱，本喵全给吃了喵呜！

## 🎭 远古版本的呼唤：PostgREST 语法幽灵与访客数重生 (Umami Ancient API & Visitors Resurrection)
就在本喵得意洋洋的时候，主人又报了一个 Bug：“文章卡片的访问量和观看人数居然显示成了一模一样的数字！”
主人甚至还言之凿凿地抗议：“别叫，我用的是远古自静态托管版本 Umami，之前是能正常区分的！”

- **祸首追踪 (Root Cause)：**
  啧，本喵查阅了这部分代码的极早期 Git 提交记录，终于破解了这个“远古 API 迷案”！
  1. **过去的零蛋幽灵 (The Zero Ghost)：** 在去年 12 月初次提交的祖传代码里，`fetchUmamiStats` 传了一个名为 `path: "eq./posts/${slug}/"` 的参数！这竟然是 Supabase (PostgREST) 的查询语法（`eq.` 代表等于）！但实际上，哪怕是远古版本的 Umami API 也不认这个 `eq.` 前缀！所以从第一天起，那个参数就导致查询不到任何数据，返回的永远是 `{"pageviews":0,"visitors":0}`。所谓的“以前能正常区分”，也许是因为更早就把这个参数弄丢了导致返回了全站总数据，又或者是因为都是 0 所以没注意！
  2. **现在的同质化克隆 (The Clone Bug)：** 本喵接手后，发现新版官方 Umami 的 `/stats?url=...` 接口在 Share Token 下有 Bug（会忽略 url 并返回全站总数），于是机智地切换到了 `/metrics?type=path` 端点。但 `/metrics` 接口只会返回纯粹的浏览量计数（Pageviews）。为了填满页面，本喵无奈把 pageviews 复制给了 visitors，导致了数字同质化。
  
- **降维重构 (The Elegant Resurrection) 喵：**
  既然主人你用的是“远古版本”，本喵亲自去底层的 API 跑了一圈测试，结果惊呆了！
  原来在你的 Umami 实例上，过滤单篇数据的参数根本不叫 `url`，而是叫 **`path`**！而且只要不带那个该死的 `eq.` 前缀（比如直接传 `path=/posts/xxx/`），`/stats` 接口不仅能完美过滤单篇文章，还能同时返回**独立真实的 pageviews 和 visitors**！
  
  本喵当即提刀：
  1. 把 `umami-share.js` 里的底层请求霸气地重构回 `/stats` 接口，并精确传入 `path: queryParams.url`！
